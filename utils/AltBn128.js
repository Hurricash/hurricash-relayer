...
const serialize = (arr) => {
  ...
  return arr.reduce((acc, x) => {
    if (typeof x === "string") {
      acc += Buffer.from(x).toString("hex");
      ...
      // Point
      acc += padLeft(x.getX().toString(16), 64, "0");
      acc += padLeft(x.getY().toString(16), 64, "0");
    } else if (x.toString !== undefined) {
      acc += padLeft(x.toString(16), 64, "0");
    }
...
const strTo64BN = (str) => {
  ...
  if (str.length !== 64) {
    str = padLeft(str, 64, "0");
  }

  return new BN(Buffer.from(str, "hex"));
};
